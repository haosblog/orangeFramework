...
//			throw new Exception('$this->table表名未设置，请检查construct中');
	 ...
	 * @return $this
	 */
	public function setTable($targetTable) {
	...
	private function Fetch() {
		...
		if($this->cache || ENTIRE_CACHE == 1){
			$key  = md5($this->sql);
			Helper::getMemcacheInstance()->key($key)->val($data)->expire($this->expire)->set();
			$this->cache = FALSE;
		}
		*/
...
//		$this->validate($data, $validateRule);
	...
	private function _parseGroup($group, $alias = ''){
		...
		foreach($group as $key => $val){
			if(is_string($val)) {
				$sqlgroup .= ($alias ? $alias .'.' : '') . $this->parseKey($val);
...
//		}
	 ...
	 * @return mixed
	 */
	public function getFieldByID($field = '', $id = 0){
