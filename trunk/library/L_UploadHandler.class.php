...
class L_UploadHandler{
	...
	protected $error_messages = array(
		...
		8 => '上传终止',
		'post_max_size' => '提交文件数量超出限制',
		'max_file_size' => '文件大小超过100M！',
		'min_file_size' => '文件太小',
		'accept_file_types' => '文件类型错误',
	...
	function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		$this->options = array(
			...
			'accept_file_types' => '/\.(rar|zip|jpg|gif|png|doc|docx|xls|xlsx|ppt|pptx|txt)$/i',
	...
	protected function get_upload_path($file_name = null, $version = null) {
		if($file_name){
			$oldname = $file_name ? $file_name : '';

			$start = strripos($oldname, '.');
			$pointName = substr($oldname, $start);

			$file_name = date('YmdHis') . rand(0, 99) . $pointName;
		} else {
			$file_name = '';
		}
	...
	protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
		...
		if ($this->validate($uploaded_file, $file, $error, $index)) {
			...
			$file->newname = substr($file_path, strripos($file_path, '/') + 1);
			$file->deleteUrl = '';
